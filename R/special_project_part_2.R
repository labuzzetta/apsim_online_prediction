...
ar <- cbind(testing, ar.forecast)
ggplot(data = ar %>%
         mutate(Year = as.factor(lubridate::year(Date)),
                `Above Ground Biomass (g/m^2)` = Maize.AboveGround.Wt,
                `Day of Year` = lubridate::yday(Date),
                `Predicted Above Ground Biomass (g/m^2)` = ar.forecast)) + 
  geom_point(aes(x = Date, y = `Predicted Above Ground Biomass (g/m^2)`)) +
  geom_line(aes(x = Date, y = `Above Ground Biomass (g/m^2)`, col = Year))